  ...
  return (
    <div className="min-h-screen bg-stone-50 py-8 px-4">
      <div className="max-w-7xl mx-auto animate-fade-in">
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-stone-900 tracking-tight">Search Trips</h1>
          <p className="text-stone-600 mt-2">Find available rides for your journey</p>
        </div>
        ...
        <div className="card p-5 sm:p-6 mb-8">
          <form onSubmit={handleSearch} className="space-y-4">
            ...
            >
              {loading ? <><span className="spinner" /> Searching...</> : 'Search Trips'}
            </button>
          </form>
        </div>
        ...
        {((sourceLocation?.lat && sourceLocation?.lng) || (destinationLocation?.lat && destinationLocation?.lng)) && (
          <div className="mb-8">
            <div className="card p-4">
              <h2 className="text-lg font-semibold text-stone-900 mb-3">Route Preview</h2>
              <MapView
              ...
              {sourceLocation && destinationLocation && (
                <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                  <div className="flex items-start space-x-2">
                    ...
                    <div>
                      <div className="font-medium text-stone-900">Pickup</div>
                      <div className="text-stone-600">{sourceLocation.address}</div>
                    </div>
                  </div>
                  <div className="flex items-start space-x-2">
                    <div className="w-3 h-3 bg-red-500 rounded-full mt-1 flex-shrink-0"></div>
                    <div>
                      <div className="font-medium text-stone-900">Drop-off</div>
                      <div className="text-stone-600">{destinationLocation.address}</div>
                    </div>
                  </div>
                </div>
              )}
        ...
        {successMessage && (
          <div className="mb-6 p-4 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-700 text-sm" role="status">
            {successMessage}
          </div>
        )}
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-stone-600">Searching for trips...</p>
          </div>
        )}
        ...
        {!loading && hasSearched && trips.length === 0 && !error && (
          <div className="card p-8 sm:p-12 text-center">
            ...
            <h2 className="text-xl font-semibold text-stone-900 mb-2">No trips found</h2>
        ...
        {!loading && !hasSearched && (
          <div className="card p-8 sm:p-12 text-center">
            ...
            <h2 className="text-xl font-semibold text-stone-900 mb-2">Start Your Search</h2>
            <p className="text-stone-600">Enter your source and destination to find available trips</p>
          </div>
        )}
      </div>
    </div>
